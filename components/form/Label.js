...
export function Label({ className, fontSize, floatLabel, htmlFor, name, text }) {
  const {
    formState: { errors }
  } = useFormContext();
  ...
  const style = {
    ...
    labelClassName: ` block ml-2 text-${fontSize} text-amcovad-secondary-700 font-normal font-Inter`
  };
